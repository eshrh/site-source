...
*** Paren mode
meow-paren-mode is a meow mode for moving around, editing, and manipulating
s-expressions using smartparens-mode.

Require meow to get functions into the namespace.
#+BEGIN_SRC emacs-lisp
(require 'meow)
#+END_SRC
We need a way to exit paren mode. However, when we do we need to explicitly
disable our paren mode and enter normal mode.

TODO: write a patch for meow that allows for arbitrary mode cancellation on
ENTERING normal mode.

#+BEGIN_SRC emacs-lisp
(defun meow-paren-state-exit ()
  (interactive)
  (when meow-paren-mode
    (meow-paren-mode -1)
    (meow-normal-mode)))
#+END_SRC

Define the keymap that the state will offer. I define just the bare minimum
here, just enough to safely get out.
#+BEGIN_SRC emacs-lisp

(defvar meow-paren-state-keymap
  (let ((keymap (make-sparse-keymap)))
    (suppress-keymap keymap t)
    (define-key keymap (kbd "<escape>") 'meow-paren-state-exit)
    keymap))

(defun meow-paren-define-key (&rest args)
  (mapcar (lambda (key-def)
            (define-key meow-paren-state-keymap
              (kbd (car key-def))
              (meow--parse-def (cdr key-def))))
          args))
#+END_SRC 
meow-paren-define-key is taken straight from the meow source and is the same for
every state.

Now, I define my actual keys. I’m not really a structural editing wizard yet, so
these keys are extremely likely to change and grow as I optimize. 
#+BEGIN_SRC emacs-lisp
...
Make sure that nothing else is active while in the paren state. This is to
ensure no nasty keymap overlap.
#+BEGIN_SRC emacs-lisp
(defun meow--paren-init ()
  (when meow-paren-mode
    (when (bound-and-true-p meow-insert-mode) (meow-insert-mode -1))
    (when (bound-and-true-p meow-motion-mode) (meow-insert-mode -1))
    (when (bound-and-true-p meow-normal-mode) (meow-normal-mode -1))
    (when (bound-and-true-p meow-beacon-mode) (meow-normal-mode -1))))
#+END_SRC

Finally, define the mode. In case you missed it, Z is mapped in the normal state
keymap above to enter paren mode. 
#+BEGIN_SRC emacs-lisp
(define-minor-mode meow-paren-mode
  "Meow paren mode."
  :init-value nil
  :lighter " [P]"
